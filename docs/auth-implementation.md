...
### What the Go API must do (short checklist)
- Require a valid Clerk session token on all admin endpoints (Bearer JWT).
- Verify JWT signature and claims using Clerk’s JWKS.
- Attach user claims to request context (user ID, email, org/roles if used).
- Provide `GET /api/me` for the frontend to validate auth + load identity.
- Return `401 Unauthorized` for missing/invalid tokens; `403` for denied access.
...
Notes:
- Use the Clerk user ID as `id`.
- `name` can be derived from Clerk claims or user profile.
- Return `roles` only if you implement RBAC (optional).
